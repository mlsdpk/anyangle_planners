...
public:
  ...
  explicit skip_vector(Allocator const &alloc) noexcept(
...
private:
  ...
  std::enable_if_t<std::is_pod<U>::value> destroy() noexcept
  {
  }
  ...
  template <typename U = T>
  std::enable_if_t<std::is_pod<U>::value> move_to(T const *b, std::size_t s, T *ptr) noexcept;

  template <typename U = T>
  std::enable_if_t<!std::is_pod<U>::value && std::is_nothrow_move_constructible<U>::value> move_to(
      T *b, std::size_t s, T *ptr) noexcept(std::is_nothrow_destructible<T>::value);

  template <typename U = T>
  std::enable_if_t<!std::is_nothrow_move_constructible<U>::value> move_to(
      T const *b, std::size_t s, T *ptr) noexcept(std::is_nothrow_copy_constructible<T>::value &&
                                                  std::is_nothrow_destructible<T>::value);
...
skip_vector<T, block_size, Allocator>::skip_vector() noexcept : skip_vector(Allocator())
{
}

template <typename T, std::size_t block_size, typename Allocator>
skip_vector<T, block_size, Allocator>::skip_vector(Allocator const &alloc) noexcept(
    std::is_nothrow_copy_constructible<T>::value)
  : Allocator(alloc)
{
}
...
template <typename T, std::size_t block_size, typename Allocator>
skip_vector<T, block_size, Allocator>::skip_vector(skip_vector &&v) noexcept
  : m_ptr(v.m_ptr), m_end(v.m_end), m_storage_size(v.m_storage_size)
{
  v.m_ptr = nullptr;
}

template <typename T, std::size_t block_size, typename Allocator>
skip_vector<T, block_size, Allocator>::~skip_vector() noexcept(
    std::is_nothrow_destructible<T>::value)
{
  if (m_ptr)
  {
...
template <typename T, std::size_t block_size, typename Allocator>
T &skip_vector<T, block_size, Allocator>::operator[](std::size_t idx) noexcept
{
  assert(idx < m_end);
...
template <typename T, std::size_t block_size, typename Allocator>
T const &skip_vector<T, block_size, Allocator>::operator[](std::size_t idx) const noexcept
{
  assert(idx < m_end);
  assert((idx & block_mask) != 0);
  return m_ptr[idx];
...
template <typename T, std::size_t block_size, typename Allocator>
T &skip_vector<T, block_size, Allocator>::back() noexcept
{
  assert(!empty());
  return m_ptr[m_end - 1];
}
template <typename T, std::size_t block_size, typename Allocator>
T const &skip_vector<T, block_size, Allocator>::back() const noexcept
{
  assert(!empty());
  return m_ptr[m_end - 1];
}
...
template <typename T, std::size_t block_size, typename Allocator>
template <typename U>
std::enable_if_t<!std::is_pod<U>::value> skip_vector<T, block_size, Allocator>::destroy() noexcept(
    std::is_nothrow_destructible<T>::value)
{
  auto i = m_end;
...
template <typename T, std::size_t block_size, typename Allocator>
template <typename U>
std::size_t skip_vector<T, block_size, Allocator>::push_back(U &&u)
{
...
template <typename T, std::size_t block_size, typename Allocator>
void skip_vector<T, block_size, Allocator>::pop_back() noexcept(
    std::is_nothrow_destructible<T>::value)
{
  assert(m_end);
...
template <typename T, std::size_t block_size, typename Allocator>
template <typename U>
std::size_t skip_vector<T, block_size, Allocator>::grow(U &&u)
{
...
template <typename T, std::size_t block_size, typename Allocator>
template <typename U>
std::enable_if_t<std::is_pod<U>::value> skip_vector<T, block_size, Allocator>::move_to(
    T const *b, std::size_t s, T *ptr) noexcept
{
  std::copy(b, b + s, ptr);
}
...
template <typename T, std::size_t block_size, typename Allocator>
template <typename U>
std::enable_if_t<!std::is_pod<U>::value && std::is_nothrow_move_constructible<U>::value>
skip_vector<T, block_size, Allocator>::move_to(T *b, std::size_t s, T *ptr) noexcept(
    std::is_nothrow_destructible<T>::value)
{
...
skip_vector<T, block_size, Allocator>::move_to(T const *b, std::size_t s, T *ptr) noexcept(
    std::is_nothrow_copy_constructible<T>::value && std::is_nothrow_destructible<T>::value)
{
  std::size_t i;
  try
  {
...
template <typename T, std::size_t block_size, typename Allocator>
bool skip_vector<T, block_size, Allocator>::empty() const noexcept
{
  return size() == 0;
}

template <typename T, std::size_t block_size, typename Allocator>
std::size_t skip_vector<T, block_size, Allocator>::size() const noexcept
{
  return m_end;
}
...
template <std::size_t block_size, typename V, typename Allocator = std::allocator<V>>
class payload
{
public:
  ...
  void push_back(U &&u)
  {
    m_storage.push_back(std::forward<U>(u));
  }
  ...
  void move(std::size_t from, std::size_t to) { m_storage[to] = std::move(m_storage[from]); }
...
public:
  ...
  explicit prio_queue(Compare const &compare, Allocator const &a) : Compare(compare), m_storage(a)
  {
  }
  ...
  std::enable_if_t<std::is_same<X, void>::value> push(U &&u);

  template <typename U, typename X>
  std::enable_if_t<!std::is_same<X, void>::value> push(U &&key, X &&value);

  template <typename U = V>
  std::enable_if_t<std::is_same<U, void>::value, value_type const &> top() const noexcept;

  template <typename U = V>
  std::enable_if_t<!std::is_same<U, void>::value, std::pair<T const &, U &>> top() noexcept;

  void pop() noexcept(std::is_nothrow_destructible<T>::value);

  template <typename U = V>
  std::enable_if_t<!std::is_same<U, void>::value> reschedule_top(T t);
...
template <typename U, typename X>
inline std::enable_if_t<std::is_same<X, void>::value>
prio_queue<block_size, T, V, Compare, Allocator>::push(U &&u)
{
...
template <typename U, typename X>
inline std::enable_if_t<!std::is_same<X, void>::value>
prio_queue<block_size, T, V, Compare, Allocator>::push(U &&key, X &&value)
{
...
template <std::size_t block_size, typename T, typename V, typename Compare, typename Allocator>
template <typename U>
inline void prio_queue<block_size, T, V, Compare, Allocator>::push_key(U &&key)
{
...
template <std::size_t block_size, typename T, typename V, typename Compare, typename Allocator>
inline void prio_queue<block_size, T, V, Compare, Allocator>::pop() noexcept(
    std::is_nothrow_destructible<T>::value)
{
  assert(!empty());
  std::size_t idx = 1;
  ...
  for (;;)
  {
    ...
    auto const sibling_offset =
        rollbear_prio_q_unlikely(address::is_block_leaf(idx)) ? address::block_size : 1;
...
template <std::size_t block_size, typename T, typename V, typename Compare, typename Allocator>
template <typename U>
inline std::enable_if_t<std::is_same<U, void>::value, T const &>
prio_queue<block_size, T, V, Compare, Allocator>::top() const noexcept
{
  assert(!empty());
  return m_storage[1];
}
...
template <std::size_t block_size, typename T, typename V, typename Compare, typename Allocator>
template <typename U>
inline std::enable_if_t<!std::is_same<U, void>::value, std::pair<T const &, U &>>
prio_queue<block_size, T, V, Compare, Allocator>::top() noexcept
{
  assert(!empty());
...
prio_queue<block_size, T, V, Compare, Allocator>::reschedule_top(T t)
{
  assert(!empty());
...
template <std::size_t block_size, typename T, typename V, typename Compare, typename Allocator>
size_t prio_queue<block_size, T, V, Compare, Allocator>::do_reschedule_top(T t) noexcept(
...
template <std::size_t block_size, typename T, typename V, typename Compare, typename Allocator>
inline bool prio_queue<block_size, T, V, Compare, Allocator>::empty() const noexcept
{
  return m_storage.empty();
}

template <std::size_t block_size, typename T, typename V, typename Compare, typename Allocator>
inline std::size_t prio_queue<block_size, T, V, Compare, Allocator>::size() const noexcept
{
  return m_storage.size() - (m_storage.size() + address::block_size - 1) / address::block_size;
}

template <std::size_t block_size, typename T, typename V, typename Compare, typename Allocator>
inline bool prio_queue<block_size, T, V, Compare, Allocator>::sorts_before(
    value_type const &lv, value_type const &rv) const noexcept
{
...
template <std::size_t blocking>
inline std::size_t heap_heap_addressing<blocking>::child_of(std::size_t node_no) noexcept
{
...
template <std::size_t blocking>
inline std::size_t heap_heap_addressing<blocking>::parent_of(std::size_t node_no) noexcept
{
...
template <std::size_t blocking>
inline bool heap_heap_addressing<blocking>::is_block_root(std::size_t node_no) noexcept
{
  return block_offset(node_no) == 1U;
}

template <std::size_t blocking>
inline std::size_t heap_heap_addressing<blocking>::block_offset(std::size_t node_no) noexcept
{
  return node_no & block_mask;
}

template <std::size_t blocking>
inline std::size_t heap_heap_addressing<blocking>::block_base(std::size_t node_no) noexcept
{
  return node_no & ~block_mask;
}

template <std::size_t blocking>
inline bool heap_heap_addressing<blocking>::is_block_leaf(std::size_t node_no) noexcept
{
  return (node_no & (block_size >> 1)) != 0U;
}

template <std::size_t blocking>
inline std::size_t heap_heap_addressing<blocking>::child_no(std::size_t node_no) noexcept
{
